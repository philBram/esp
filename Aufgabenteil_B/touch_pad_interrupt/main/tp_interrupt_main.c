        ...
        if (filter_mode == 0) {
            ...
            for (int i = 0; i < TOUCH_PAD_MAX; i++) {
                if (s_pad_activated[i] == true) {
                    ...
                    //ledc_duty toggle between 0 and 8191 when touch pad is touched in interrupt mode
                    if (ledc_duty == 0) ledc_duty = ledc_duty_max;
                    else ledc_duty = 0;
                    // Set duty
            ...
            //change duty cycle led in filter mode
            ...
            for (int i = 0; i < TOUCH_PAD_MAX; i++) {
                ...
                if (value < s_pad_init_val[i] * TOUCH_THRESH_PERCENT / 100) {
                    ...
                    //change led duty cycle dependent on which Touch pad pin was touched
                    if (i == TOUCH_PAD_MAX - 1) ledc_duty = ledc_duty_max;
                    else ledc_duty = i * ledc_duty_steps;
                    // Set duty
                    ESP_ERROR_CHECK(ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, ledc_duty));
                    // Update duty to apply the new value
                    ESP_ERROR_CHECK(ledc_update_duty(LEDC_MODE, LEDC_CHANNEL));
